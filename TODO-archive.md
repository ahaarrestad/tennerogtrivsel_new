...
- [x] **Fjern TS 7-blokkeringen i CI** ([plan](docs/plans/archive/2026-08-09-ts7-blokkering.md))
  ...
  - **Andre review-runde (etter implementering) fant to stille dødsmåter for watchdogen** — begge fikset: (1) enhver probe-feil ble tolket som «fortsatt blokkert», så registry-nedetid eller et npm-flagg som bytter navn ville gitt permanent grønn, permanent død watchdog; nå kreves `ERESOLVE` i outputen for den konklusjonen, ellers rødt. (2) Dedup mot `--state all` gjorde varselet til engangs — lukket noen issuen uten å fjerne `ignore`-regelen, sa watchdogen aldri fra igjen; nå dedupes kun mot åpne issues, og jobben avbryter tidlig med `::notice::` hvis `ignore`-regelen er borte fra `dependabot.yml` (da skal workflowen slettes). Alle fire grener verifisert med stubbet `npm`.
