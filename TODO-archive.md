...
- [x] **Lås Node-versjonen på tvers av lokalt og CI** ([spec](docs/designs/archive/2026-09-25-node-version-lock.md)) ([plan](docs/plans/archive/2026-09-25-node-version-lock.md))
  ...
  - **Verifisert etter merge (2026-09-25):** CI/CD på main grønn med `Resolved .nvmrc as 24.21.0` (treff i runnerens toolcache); `node-version-watch.yml` via `workflow_dispatch` grønn uten issue; lokal nvm-default satt til 24.21.0.
  - **Dependabot:** kan ikke trigges manuelt (ingen API, jobbene kan ikke re-kjøres). Dependabots npm-image installerer nyeste Node 24.x, altså innenfor range. Første ukentlige kjøring for `/` med `engine-strict` sjekkes. **Kjent fremtidig risiko:** når Dependabot går over til Node 26, stopper taket `<25` rot-jobbene stille — oppgrader til 26 når watchdogen melder `node-lts-watch`, eller løft taket.
...
- [x] **Cache-bust kart-tiles** ([plan](docs/plans/archive/2026-09-25-cache-bust-kart-tiles.md))
  ...
  - **Runbook:** `sikkerhet.md`, `aws-infrastruktur.md` og kommentaren i `deploy.yml` sier nå: invalider `/tiles/*`, verifiser ren edge, bump **deretter** `v` — i motsatt rekkefølge får nettleserne det dårlige objektet på nytt under ny URL.
  - **Oppfølging i backlog:** «Kort nettleser-cache for `/tiles/*`» (egen response headers policy som overstyrer `Cache-Control`).

- [x] **Stabiliser ustabile tester** ([spec](docs/designs/archive/2026-09-22-stabiliser-ustabile-tester.md)) ([plan](docs/plans/archive/2026-09-22-stabiliser-ustabile-tester.md))
  - **Tre last-avhengige tester, tre ulike mekanismer.** Kun én av dem (A) har faktisk feilet i CI (1 av 5 røde kjøringer siste 60 — resten var npm audit/build). Alle tre ble målt før noe ble endret; TODO-punktets hypoteser ble delvis tilbakevist.
  - **A — `accessibility.spec.ts` (/admin):** `waitForLoadState('networkidle')` ventet på en *seriell* kjede av tre Google-verter (`gapi.load` → `gapi.client.init` → discovery-dokumenter → stille token-forespørsel): målt `load` 0,7–1,9 s, `networkidle` 2,7–3,6 s. Historikken viste ping-pong (`8bae670` → `185a044` → `24fc6b4`) der `networkidle` ble stående som dev-reload-workaround etter at warm-upen i `global-setup.ts` allerede hadde tatt over den jobben. Fiks: hjelperen `ventTilKlarForAxe` — `domcontentloaded` + `main` + alle `link[rel~=stylesheet]` har `.sheet` (10 s eget timeout) + `document.fonts.ready`. Første utkast brukte `load`; mekanismetesten (35 s `page.route`-forsinkelse på `*google*`) viste at `load` også timet ut, fordi `<script async defer>` inngår i `load`. Kun `domcontentloaded` er uavhengig av de eksterne skriptene. Gjelder alle a11y-tester, ikke bare admin.
  - **B — `links.spec.ts`:** «Execution context was destroyed» fra 2026-09-15 lot seg *ikke* reprodusere (kald `node_modules/.vite/deps` + load 16, 8/8 grønne, ingen optimizer-/reload-linjer i dev-loggen). Testen ble herdet uansett: hrefs hentes i ett `page.evaluate`, ikke-tom-sjekk på tjeneste-lenkene, og feilmeldingene sier hvilken side det gjaldt. Warm-up-utvidelsen i `global-setup.ts` ble droppet (plan 3.4).
  - **C — `data-validation.test.ts`:** TODO-punktets rotårsak («manglende `.astro/` i worktree») var **feil** — korrelasjonen dagen før var last, ikke katalogen (retest: `.astro/` fjernet, load 2 → 1609/1609). Kald sidecache ble også utelukket. Faktisk årsak: `import('../content.config')` drar inn `astro/loaders` (~780 ms inne i vitest), og hele importen belastes 5 s-`testTimeout`. Fiks: `vi.mock('astro/loaders')` i `data-validation.test.ts` og `content.config.test.ts` (ingen test bruker glob-loaderen) → 23 ms. `astro sync` i `setup-worktree.sh` er derfor *ikke* lagt til.
  - **C-variant funnet under verifisering:** `getSettings.test.ts` importerte `sync-data.js` (googleapis + sharp, ~0,6 s) inne i testkroppen og sprakk under load 27. Samme fiks — mock av de fire tunge avhengighetene, som `sync-data.test.js` allerede gjør → 129 ms.
  - **Utenfor scope, notert som backlog:** WebKit + axe er CPU-bundet (`goto` 3–4 s, `analyze()` 4–5 s per side ved load 7); under load ~22 med fire WebKit-workere feiler gammel og ny variant likt. `csp-check.spec.ts` bruker fortsatt `networkidle`. `admin-init.test.js` (240 ms isolert) sprakk én gang ved load 27, aldri ved ≤ 23 — ikke en tung import, ikke endret.
  - **Verifisert 2026-09-22:** mekanismetest gammel → timeout / ny → pass ✓; a11y+links chromium ×5 under load 19–26: 45/45 ✓; a11y Mobile Safari ×5 (uten kunstig last): 35/35 ✓; a11y mot `preview` (CI-stien) chromium ×5 under load 22: 35/35 ✓; `npx vitest --run` ×3 under load 6→23: 1609/1609 alle tre ✓. Ingen timeout økt, ingen `retries`/`skip`. Review-loop: CLEAN etter én runde (fire Minor fikset).
  - **Sideeffekt for agent-økter:** `astro preview` daemoniserer nå også under AI-agent (som `astro dev`). Løsning: forhåndsstart `npx astro preview --port 4322`, kjør Playwright med `PORT=4322` og `--workers=4`, rydd med `npx astro preview stop`.

- [x] **CI: `dependabot-rebase.yml` gjenkjenner ikke Dependabot-PR-er** ([plan](docs/plans/archive/2026-09-21-dependabot-rebase-deteksjon.md))
  - **Symptom:** sjekken `[ "$author" = "dependabot[bot]" ]` traff aldri, fordi `gh pr list --json author` rapporterer boter som `{"is_bot":true,"login":"app/dependabot"}` (siden gh 2.0). Alle Dependabot-PR-er gikk i `else`-grenen og ble rebaset via `update-branch`-API-et med `MY_GITHUB_PAT`. Loggene viser at Dependabot-grenen aldri hadde kjørt — feilen stammer fra refaktoreringen `50fe12c` (2026-03-01).
  - **Konkret utslag:** PR #472 (2026-09-19) ble åpnet 15:55:54 og PAT-rebaset 15:55:58 fordi #470 merget i samme sekund. Den nye commiten var usignert med brukeren som committer; `dependabot/fetch-metadata` i `dependabot-auto-merge.yml` nektet («Dependabot's commit signature is not verified»), auto-merge ble aldri aktivert, og senere `synchronize`-hendelser hadde `actor = ahaarrestad` så jobben ble hoppet over. PR-en sto til den ble merget manuelt 2026-09-20.
  - **Fiksen:** `case "$author" in "dependabot[bot]"|"app/dependabot")` — matcher både formen `gh --json author` gir og formen REST-API-et (`user.login`) og `github.actor` bruker. Eksplisitt login-match ble valgt framfor `.author.is_bot` så `@dependabot rebase` ikke sendes til andre boter. Ingen andre linjer i workflowen er endret.
  - **Reviewfunn (runde 1):** `run:` uten `shell:` kjører som `bash -e`, og `while`-løkken står i en pipeline-subshell — én feilende `gh pr comment` ville avbrutt resten av PR-ene. Lagt til `|| echo "  Skipped (comment failed)"`, uten `2>/dev/null` så feilen er diagnostiserbar i loggen. Kommentaren i workflowen sa først at `[bot]`-formen kom fra «eldre gh»; rettet til REST-API/`github.actor`.
  - **Non-goals (bevisst):** ingen sikkerhetsnett i `dependabot-auto-merge.yml` for PR-er der `fetch-metadata` nekter, ingen GitHub App-migrering, ingen `mergeStateStatus`-filtrering (rapporterer `UNKNOWN` rett etter push). PAT-omskriving av commits generelt hører til «Sikkerhetshardening» Task 3.
  - **Verifisert:** YAML-parse ✓; `bash -n` på `run`-blokken ✓; `case`-uttrykket kjørt mot `dependabot[bot]`, `app/dependabot` (→ Dependabot-gren) og `ahaarrestad`, `app/github-actions`, `github-actions[bot]` (→ `update-branch`-gren) ✓; tørrkjøring av hele `run`-blokken mot ekte `gh pr list`-data for de siste PR-ene med stubbet `gh pr comment` ✓. Review-loop: CLEAN etter to runder.
  - **Gjenstår manuelt etter merge (ved neste Dependabot-PR):** V1 loggen til «Auto-Rebase Open PRs» viser «commenting @dependabot rebase» for PR-en; V2 Dependabot force-pusher en rebase; V3 head-commiten har `verification.verified == true`; V4 auto-merge aktiveres på `synchronize`-hendelsen og PR-en merger uten manuell inngripen.

- [x] **Innholdsdeploy skal ikke blokkeres av avvik som ikke kan handles på** ([spec](docs/designs/archive/2026-09-15-innholdsdeploy-blokkering.md)) ([plan](docs/plans/archive/2026-09-15-innholdsdeploy-blokkering.md))
  - **Utløsende hendelse:** 2026-09-15 stoppet `npm audit --audit-level=critical` en Drive-oppdatering på GHSA-26w7-cxv4-gfx2 (Astro RCE). Ingen kode var endret — advisory-en ble publisert etter forrige grønne kjøring. Redaktøren kunne ikke publisere innhold, og ingenting hun kunne gjøre i den kjøringen ville fjernet sårbarheten.
  - **Prinsippet:** *en gate skal blokkere der funnet er handlingsbart i den kjøringen.* På `repository_dispatch`-stien er lockfilen byte-identisk med den som alt kjører i prod, så en blokkering fjerner ingen sårbarhet — den hindrer bare innholdspublisering. På `push`/`workflow_dispatch` kan treet faktisk endres, og der blokkerer gaten som før.
  - **Selve fiksen er to linjer** i `.github/workflows/deploy.yml`: `continue-on-error: ${{ github.event_name == 'repository_dispatch' }}` på audit-steget i `build` og `update-lambda`. `continue-on-error` er en av de få **step**-nøklene som tar uttrykk. `steps.audit.outcome` (før `continue-on-error`) er det rapporteringssteget leser — `conclusion` ville alltid vært `success`.
  - **Funnet går ikke tapt:** gaten pipes gjennom `tee` til `$RUNNER_TEMP/audit.txt`, og et oppfølgingssteg i `build` skriver rapporten til jobbsammendraget og åpner en GitHub-issue med etiketten `sikkerhet-auto`. `PIPESTATUS`-linja er kritisk: default-shellen er `bash -e` **uten** `pipefail`, så uten `exit ${PIPESTATUS[0]}` ville `tee`-ens exit 0 skjult auditfeilen og gaten sluttet å gate i det hele tatt. Rapporteringssteget har selv `continue-on-error: true` — det skal aldri kunne velte jobben det nettopp slapp gjennom.
  - **`build` fikk jobbnivå-`permissions`** (`contents: read`, `issues: write`). Jobbnivå-permissions **erstatter** toppnivå-settet, derfor må `contents: read` gjentas. Rapporteringen ligger kun i `build`: `update-lambda` auditerer samme lockfile i samme kjøring, så en issue til ville vært duplikatstøy.
  - **`scheduled-audit.yml` hevet fra ukentlig til daglig** (`0 6 * * *`). `high`-båndet dekkes ikke av noen gate i `deploy.yml`, så den planlagte auditen er eneste deteksjon der — en ukes vindu var for langt når dispatch-stien nå slipper gjennom.
  - **Dedupen ble bygget, utvidet tre ganger under review, og til slutt revet ned igjen.** Mellomvarianten holdt et `<!-- avtrykk: … -->`-felt med advisory-URL-ene så avvik nummer to kunne varsles mens issuen om det første sto åpen. Den kostet ~60 linjer bash og produserte tre feil av seg selv: `gh issue edit --body-file` slettet triage-notater og `sed`-ankeret brøt stille på CRLF (runde 2); på et **offentlig** repo kunne hvem som helst kommentere en markør som matchet og dermed slå av et sikkerhetsvarsel (runde 3); og registry-nedetid ga tomt avtrykk → varsel ved hver eneste publisering. Gjeldende mekanikk er tre linjer: finnes en åpen issue med etiketten, gjør ingenting. Konsekvensen — avvik nummer to får ikke eget varsel — er et bevisst bytte: den som setter seg ned og fikser auditen kjører `npm audit` og ser hele bildet uansett.
  - **Brukerens inngripen styrte utfallet.** Etter tre reviewrunder på rapporteringsbash-en spurte brukeren om vi faktisk løste problemet eller drev med fjas. Svaret var nei: kjernefiksen var korrekt fra første commit, og all kompleksiteten hadde samlet seg i et hjørne med marginal verdi. Forenklingen fjernet 115 linjer, la til 49, og eliminerte alle tre Important-funnene fra runde 2–3 ved å slette koden de bodde i.
  - **Dokumentasjon:** `docs/architecture/sikkerhet.md` (hvorfor gaten er betinget, dedup-valget og prisen for det, 50 kB-avkortingen mot GitHubs tak på 65 536 tegn, og at fork-PR-er ikke får audit-gate). YAML-utdraget der er merket som forenklet og må ikke kopieres uten `PIPESTATUS`-linja. Backlog-oppgaven om issue-rapportering i `scheduled-audit` peker nå på etikett-mønsteret — med eksplisitt advarsel mot å gjenoppfinne avtrykket.
  - **Verifisert:** YAML-parse på begge workflow-filene ✓; `bash -n` på begge `run`-blokkene ✓; gaten kjørt under `bash -e` uten `pipefail` (rc 1 ved auditfeil, rc 0 ellers) ✓; alle dedup-grener gjennomgått med stubbet `gh` ✓; 80 kB rapport gir 50 807 bytes issue-kropp med intakt fence ✓. Review-loop: CLEAN etter fire runder.
  - **Gjenstår manuelt etter merge:** bekreft at en `push`-kjøring på main er grønn, og at en `repository_dispatch` fullfører og deployer.

- [x] **Kartet viser «API KEY REQUIRED» — CARTO krever nå nøkkel for basemap-tiles** ([spec](docs/designs/archive/2026-09-08-carto-api-nokkel.md)) ([plan](docs/plans/archive/2026-09-08-carto-api-nokkel.md))
  - **Symptom:** tiles på kontaktsiden var stemplet med et diagonalt vannmerke «API KEY REQUIRED / carto.com/basemaps/apikey». CARTO har innført nøkkelkrav for `basemaps.cartocdn.com`. Degraderingen var usynlig for testpakken: endepunktet svarte fortsatt `200 OK` med gyldig `image/png` i normal størrelse (17212 B uten nøkkel vs. 19689 B med).
  - **Beslutning:** CARTO-nøkkel med `/tiles/*`-proxyen beholdt (proxyen skjuler besøkendes IP for tredjepart — GDPR-begrunnelsen står fortsatt). Nøkkelen bor i GitHub Secrets (`CARTO_API_KEY`), aldri i git, aldri i klient-JS. CARTO Basemap Terms §9.c.iii-risikoen er dokumentert, ikke ignorert.
  - **`scripts/cloudfront-strip-tiles-prefix.js`:** funksjonen setter nå `?key=` fra en placeholder `__CARTO_API_KEY__` i tillegg til path-omskrivingen. Nøkkelen settes *etter* at nettleseren har sendt sin forespørsel, så klienten ser den aldri.
  - **`scripts/setup-cloudfront-functions.mjs`:** ny `injectCartoKey()` bytter placeholderen mot `process.env.CARTO_API_KEY` rett før opplasting og **kaster** hvis nøkkelen mangler — uten den ville CARTO levert vannmerkede tiles. Ny `assertCartoKeyAvailable()` kjøres før deploy-løkka, så en manglende nøkkel ikke etterlater en delvis kjørt deploy. Koden må innom disk (`--function-code fileb://` krever sti); tmp-fila skrives i en `mkdtempSync`-katalog (0700) med `mode: 0o600` og slettes i `finally`.
  - **Origin request policy `carto-tiles-key-forward`** (`f6aa3531-3882-45d0-97a7-8400b29263fb`) opprettet og knyttet til `/tiles/*` på både prod (`E9Z51DQB2K1G4`) og test (`E2WXX7ZUR5NNP3`). Nødvendig fordi `Managed-CachingOptimized` har `QueryStringBehavior: none` — `key` ville blitt strippet før origin, også når funksjonen selv satte den.
  - **Review-loopen fant to sikkerhetshull som ikke var i den opprinnelige planen:**
    - **Cache-forgiftning via videresendt `Referer`:** cache-nøkkelen for `/tiles/*` er kun pathen. Videresendte vi klientens egen `Referer`, kunne hvem som helst hotlinke en tile-URL fra et fremmed domene, få et vannmerket svar fra CARTO, og få det cachet i 24 t for alle ekte besøkende. Verifisert direkte mot `basemaps.cartocdn.com` (ikke gjennom vår proxy, som ville forgiftet den ekte cachen). Funksjonen setter derfor `Referer` selv. Bonus: klientens `Referer` er full side-URL, så videresending hadde lekket hver besøkendes side-URL til CARTO — stikk i strid med hvorfor vi proxyer.
    - **`Host` er ikke unik:** første fiks utledet `Referer` av `Host`, i den tro at CloudFronts alias-validering gjorde den entydig. Prod har **seks** aliaser (apex + www for .no/.net/.com) pluss `*.cloudfront.net`, og kun ett domene registreres på nøkkelen. www-redirecten i `sitemap_redirect` redder oss ikke: CloudFront tillater kun én viewer-request-funksjon per behavior, og `/tiles/*` har `strip-tiles-prefix` — verifisert at `https://tennerogtrivsel.com/tiles/…` svarer `200`, ikke `301`. `Host` mappes nå til én av to konstanter, så ukjente/framtidige aliaser feiler trygt til prod-domenet.
  - **`astro.config.mjs`:** dev-proxyen legger på `key` fra `.env` via `loadEnv(mode, root, '')` (Vite laster `.env` etter at configen er evaluert). Mangler nøkkelen lokalt, rewrites pathen som før — dev degraderes, men krasjer ikke.
  - **`.github/workflows/deploy.yml`** sender `CARTO_API_KEY` til CF-funksjons-steget; `.env.example` dokumenterer variabelen. **Steget ble samtidig flyttet fremst i `deploy`-jobben:** det sto sist, etter invalideringen av `/*`, slik at cachen ble tømt mens den gamle funksjonen uten nøkkel fortsatt var live på edge. Trafikk i det vinduet ville repopulert `/tiles/*` med vannmerkede tiles og låst dem i 24 t — fiksen ville altså sett ut som om den ikke virket. Steget leser kun `scripts/*.js` fra checkouten og er uavhengig av bygget, så det ligger nå først og får maksimal propageringstid før cachen tømmes.
  - **Dokumentasjon:** `docs/architecture/sikkerhet.md` (nøkkelens vei fra secret til CDN, cache-forgiftningsvektoren, Host-normaliseringen, og at **access logging må forbli av** — ellers havner nøkkelen i loggene), `docs/architecture/aws-infrastruktur.md` (deploy-kommando + fail-fast), `docs/runbooks/supply-chain-incident.md` (`CARTO_API_KEY` i secrets-tabellen).
  - **Verifisert:** 1599/1599 tester ✓ (51 filer); `cloudfront-strip-tiles-prefix.js` 9/9 branch (100 %), `setup-cloudfront-functions.mjs` 6/6 (100 %), alle filer ≥ 80 %; `npm run lint` rent; `npm run build:ci` grønn; `git grep` på nøkkelen tom, og `dist/` fri for både nøkkel og placeholder. Review-loop: CLEAN etter fire runder.
  - **Ikke med i denne oppgaven:** deteksjon av stille tile-degradering, og veivalget vektor vs. selvhost — begge egne backlog-oppgaver.


- [x] **Fjern TS 7-blokkeringen i CI** ([plan](docs/plans/archive/2026-08-09-ts7-blokkering.md))
  - Dependabot-gruppen `version-updates` feilet ukentlig på `npm ci` (ERESOLVE) fordi `typescript@7` er blokkert av **to** peer-krav: `@astrojs/check@0.9.10` (`^5.0.0 || ^6.0.0`) og `typescript-eslint@8.65` (`>=4.8.4 <6.1.0`, støtte tracket mot TS **>=7.1**). CI-loggen viste bare den første — npm stopper på første konflikt. Blokkerte 5 urelaterte oppdateringer i samme gruppe-PR (#433).
  - **`.github/dependabot.yml`:** `ignore` for `typescript` `>=7` i npm-`/`-blokken (ikke lambda), med kommentar som navngir begge blokkeringer og peker på watchdogen for når regelen skal fjernes.
  - **`.github/workflows/blocked-upgrades-watch.yml`** (ny): ukentlig watchdog (mandag 06:30 UTC, etter `scheduled-audit`) + `workflow_dispatch`. Prøver `npm install --package-lock-only --strict-peer-deps typescript@^7`; ved suksess verifiserer den med `npm ci` + `lint` + `seed:fixtures` + `astro sync` + `check` (samme sekvens som `type-check`-jobben, ingen secrets), og åpner først da en issue. Normaltilstanden «fortsatt blokkert» gir **grønn** kjøring. Dedup via label `ts7-watch` mot **åpne** issues. Bruker `github.token` (kun `issues: write`) framfor `MY_GITHUB_PAT` — bevisst avvik, kommentert i fila.
  - **Andre review-runde (etter implementering) fant to stille dødsmåter for watchdogen** — begge fikset: (1) enhver probe-feil ble tolket som «fortsatt blokkert», så registry-nedetid eller et npm-flagg som bytter navn ville gitt permanent grønn, permanent død watchdog; nå kreves `npm error code ERESOLVE` **og** `npm error … peer typescript@` for den konklusjonen, ellers rødt. (2) Dedup mot `--state all` gjorde varselet til engangs — lukket noen issuen uten å fjerne `ignore`-regelen, sa watchdogen aldri fra igjen; nå dedupes kun mot åpne issues, og jobben avbryter tidlig med `::notice::` hvis `ignore`-regelen er borte fra `dependabot.yml` (da skal workflowen slettes).
  - **Tredje runde strammet begge fiksene:** løs `grep 'ERESOLVE'` traff også `npm warn ERESOLVE overriding peer dependency` (som står i dagens ekte probe-output), så en urelatert nettverksfeil etterpå ville fortsatt blitt lest som «blokkert»; og guard-grepen bommet på den gyldige siterte formen `- dependency-name: "typescript"`, som ville gjort watchdogen permanent taus. Begge mønstre er nå ankret/kodebaserte. Alle seks grener + fire guard-varianter verifisert med stubbet `npm`, og dagens tilstand verifisert med ekte `npm`.
  - **Uavhengig review fanget to kritiske feil i planutkastet:** (1) proben ga `exit 0` i dag — npm 11 har `strict-peer-deps=false` som default for `install`, så ERESOLVE degraderes til warning og lockfilen skrives likevel; watchdogen ville fyrt falsk issue på første kjøring. Fikset med `--strict-peer-deps`. (2) `typescript-eslint` var ikke oppdaget som blokkering, og planen påsto feilaktig at konflikten forsvinner hvis `@astrojs/check` fjernes.
  - **Verifiserte blindveier** (dokumentert i planen): `overrides`/`--legacy-peer-deps` lar installasjonen lykkes, men både `eslint` og `astro check` har harde runtime-vakter mot TS 7 — feilen flyttes bare fra install-tid til kjøretid. `@astrojs/check` kan ikke fjernes: `astro check` i Astro 7 er en wrapper som krever pakken.
  - **Verifisert:** YAML-parse på begge filene ✓; rendret bash fra `run`-blokkene syntaks-sjekket ✓ (fanget at heredoc-innhold må ligge innenfor blokk-innrykket — la om til `--body-file`); proben kjørt med workflowens eksakte kommando i temp-katalog → `EXIT=1` i dag, og den avdekker `typescript-eslint`-konflikten ✓; `npm run lint` rent; `npm test` 1568/1568 ✓ (dekning uendret — ingen kjerne-logikk berørt, så 80 %-kravet treffer ingen berørte filer).
  - **Merk:** `typescript` var eneste major-bump i PR #433, så når den ignoreres går gruppe-PR-en fra «flagget for manuell review» til auto-merge via `dependabot-auto-merge.yml:21`. I tråd med eksisterende policy for minor/patch, men effekten er ny for denne PR-en.
  - **Gjenstår manuelt:** `@dependabot recreate` på PR #433 etter merge til main, og `workflow_dispatch`-kjøring av watchdogen for å bekrefte grønn normaltilstand.
...
- [x] **Vurder drift i `csp-hashes.json`** ([plan](docs/plans/archive/2026-06-23-csp-hashes-drift.md))
  - Backlog-funn (galleri-bildeprosessering, 2026-06-07): committet `src/generated/csp-hashes.json` så ut til å avvike fra `generate-csp-hashes`. **Diagnose (reprodusert 3 måter — stale dist, fersk `build:ci`, fixtures-bygg):** ingen aktiv drift på `main`; generatoren leser inline-skript fra `dist/` (et bygg-artefakt), så «drift» er symptom på generering fra et utdatert/fremmed bygg — ikke ikke-determinisme. Den opprinnelige observasjonen skjedde midt i galleri-arbeidet med en `dist/` fra annen kildekode.
  - **Tiltak:** CI-gate i `e2e-tests`-jobben (`.github/workflows/deploy.yml`) etter fixtures-bygget: `generate-csp-hashes` + `git diff --exit-code src/generated/csp-hashes.json`. Fanger en utdatert committet hash-fil før den blokkerer CSP lokalt i `dev:secure`. Pålitelig fordi fixtures-bygg gir samme hasher som prod (datadrevne `application/json`-skript ekskluderes allerede).
  - **Review-fiks (Important):** porten sammenligner byte-eksakt, men generatorens rekkefølge kom fra `readdirSync` (ikke stabil på tvers av filsystemer) → kunne gi falske CI-feil. `generate-csp-hashes.mjs` sorterer nå output kanonisk (rekkefølge er semantisk irrelevant for CSP); hash-fila regenerert sortert; regresjonstest lagt til.
  - **Verifisert:** vitest 1565/1565 ✓ (generator-tester 18/18; global branch 89,1 %); porten passerer i post-commit-tilstand (tom `git diff`) og feiler korrekt på en stale committet fil (negativ test). Review-loop: CLEAN.
  - **Merknad:** Gemini hadde forhåndsdraftet planen; dens kjernediagnose stemte, men den siterte feil commit (`1e0c934`) for en `gallery-lightbox.js`-endring (er egentlig en arkiverings-commit) og antok fixtures==prod uten å verifisere. Begge rettet/verifisert.
...
- [x] **Button.astro: attributt-lekkasje til `<span>`** ([plan](docs/plans/archive/2026-06-22-button-attributt-lekkasje.md))
  - PR #380-funn (Gemini, medium): `{...rest}` videresendte interaktive attributter (`type`, `disabled`, `target`, `rel`) til den dynamisk valgte taggen (`<span>`/`<a>`/`<button>`), så de havnet på elementer som ikke støtter dem → ugyldig HTML. Ingen kjente kall trigget det i dag; fiksen er robusthet.
  - **Fiks** (`src/components/Button.astro`): destrukturer `type`/`disabled`/`target`/`rel` eksplisitt ut av `Astro.props` (fjernes fra `...rest`) og re-app­liser betinget — `type`/`disabled` kun på `<button>`, `target`/`rel` kun på `<a>`, ellers `undefined` (Astro utelater undefined-attributter). `href` forblir gated av `interactive`.
  - **TDD:** Innførte prosjektets første `.astro`-komponenttest (`src/components/__tests__/Button.test.ts`) via `experimental_AstroContainer` — RED (3 feilende lekkasje-tester) → GREEN (4/4). Krevde bytte av `vitest.config.ts` til Astros `getViteConfig` så `.astro`-filer transformeres under Vitest; eksisterende `astro:content`-alias, includes og coverage-terskler bevart (merges av `getViteConfig`).
  - **Verifisert:** vitest 1564/1564 ✓ (global branch 89,1 %; `Button.astro` 100 % lines / 94,4 % branch); `build:ci` ✓ (9 sider); Playwright `telefon-tab-rekkefolge` 8/8 ✓ mot preview (treffer `interactive={false}`-spennet). Review-loop: CLEAN — ingen Critical/Important, kun valgfrie minor.
