...
test.describe('Admin-panel Fase 1', () => {
  ...
  test('skal laste admin-siden og vise innloggingsknapp uten JS-feil', async ({ page }) => {
    ...
    const criticalErrors = consoleErrors.filter(e => !e.includes('401') && !e.includes('403') && !(e.includes('Content Security Policy') && e.includes('gstatic.com')) && !e.includes('[vite] Outdated Optimize Dep'));
