...
**Tile-kilde:** CartoDB Voyager (`basemaps.cartocdn.com/rastertiles/voyager/`). **Origin er Fastly, ikke CARTO selv** — `basemaps.cartocdn.com` er en CNAME til `j.sni.global.fastly.net`. Ved cache-miss hos oss går forespørselen (med `X-Forwarded-For`) til en Fastly-edge, i praksis Oslo eller Frankfurt; først ved miss også der treffer den CARTOs egen origin i US-East (`x-origin-server: bm-us-east-*`). Fastly er dermed en mottaker i kjeden på linje med CARTO, og personvernerklæringen navngir begge. Tidligere ble `tile.openstreetmap.org` brukt, men OSM blokkerer CDN-proxying (krever unik User-Agent og Referer-header, se [OSM Tile Usage Policy](https://operations.osmfoundation.org/policies/tiles/)).
