...
**Funnet går ikke tapt.** Slår `continue-on-error` inn, skriver et oppfølgingssteg i `build` audit-utdataen til jobbsammendraget og oppretter en GitHub-issue med etiketten `sikkerhet-auto`. Dedupen er todelt: etiketten finner en eventuelt åpen issue (ikke tittelen — titler redigeres under triage, og da ville dedupen brutt stille), og et `<!-- avtrykk: … -->`-felt holder settet av URL-er til advisories med `severity == "critical"`. Er settet uendret, gjøres ingenting; har et nytt avvik kommet til, kommenteres den åpne issuen i stedet for å slukes. Fasit er **siste markør i kropp og i kommentarer fra `github-actions`** — issue-kroppen skrives aldri på nytt, slik at triage-notater et menneske har lagt inn ikke går tapt. Forfatterfilteret er nødvendig fordi repoet er offentlig: uten det kunne en fremmed kommentere seg til en markør som matcher og dermed slå av varselet. Lar utdataen seg ikke tolke som et avvikssett — typisk at npm-registryet er nede, som også får gaten til å feile — dedupliseres det på en hash av rapporten i stedet, og issuen sier eksplisitt at dette kanskje ikke er et sårbarhetsfunn. Gaten selv skriver rapporten via `tee`, så issuen viser det gaten faktisk så (avkortet til 50 kB, som er GitHubs tak for issue-kropper). Steget har selv `continue-on-error: true` — det skal aldri kunne velte jobben det nettopp slapp gjennom. `build` har derfor jobbnivå-`permissions` med `issues: write`; merk at jobbnivå-`permissions` **erstatter** toppnivå-settet, så `contents: read` må gjentas der. Rapporteringen ligger kun i `build`: `update-lambda` auditerer samme lockfile i samme kjøring, så en issue til ville vært duplikatstøy.
