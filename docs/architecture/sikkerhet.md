...
- **Nettleser-cache:** CARTO sender `max-age=15552000` (180 dager). Viewer-response-funksjonen `tiles-browser-cache` overstyrer den til `public, max-age=86400` mot nettleseren på `200` og `304` (304 må med — CloudFront sender cache-objektets headere ved revalidering). Edge-cachen styres fortsatt av CARTOs header. Funksjonen deployes automatisk, men virker først når den er manuelt tilknyttet `/tiles/*`-behavioren (se `aws-infrastruktur.md`) — gjort i test og prod 2026-09-25. Uten funksjonen blir en dårlig tile liggende i nettleseren i et halvt år, og CloudFront-invalidering når ikke nettleserne. Den er en egen funksjon, ikke en response headers policy, fordi en behavior bare kan ha én policy og en kopi av `tot-security-headers` ikke ville blitt oppdatert av CSP-synken.
- **Leaflet:** Tile URL er `/tiles/{z}/{x}/{y}.png?v=2` (relativ path, fungerer i begge miljøer). `v` er cache-busting mot nettleseren: nettleseren holder en tile i maks ett døgn (se over), men tiles hentet før `tiles-browser-cache` ble tatt i bruk har 180 dager. Bump `v` for å tvinge ny henting umiddelbart — men **først etter** at edge er invalidert og verifisert ren: `v` inngår ikke i cache-nøkkelen, så en bump mens CloudFront fortsatt har en dårlig tile gir nettleserne det samme dårlige objektet på ny URL. Parameteren forwardes ikke til origin (`carto-tiles-key-forward` slipper kun `key` gjennom). Bumpet til `2` 2026-09-25 etter vannmerke-episoden.
...
**Funnet går ikke tapt.** Slår `continue-on-error` inn, skriver et oppfølgingssteg i `build` audit-utdataen til jobbsammendraget og oppretter en GitHub-issue med etiketten `sikkerhet-auto`. Dedupen er med vilje så enkel som mulig: finnes det alt en åpen issue med den etiketten, gjøres ingenting. Etikett og ikke tittel, fordi titler redigeres under triage og dedupen da ville brutt stille. Konsekvensen er at et nytt avvik som dukker opp mens issuen står åpen ikke får eget varsel — et bevisst bytte, for den som fikser auditen ser hele bildet uansett. Rapporten i issuen kommer fra `tee`-en i gaten (avkortet til 50 kB, trygt under GitHubs tak på 65 536 tegn), så den viser det gaten faktisk så. Steget har selv `continue-on-error: true` — det skal aldri kunne velte jobben det nettopp slapp gjennom. `build` har derfor jobbnivå-`permissions` med `issues: write`; merk at jobbnivå-`permissions` **erstatter** toppnivå-settet, så `contents: read` må gjentas der. Rapporteringen ligger kun i `build`: `update-lambda` auditerer samme lockfile i samme kjøring, så en issue til ville vært duplikatstøy.
