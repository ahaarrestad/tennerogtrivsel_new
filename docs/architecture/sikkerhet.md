...
- **Nettleser-cache:** CARTO sender `max-age=15552000` (180 dager). Viewer-response-funksjonen `tiles-browser-cache` overstyrer den til `public, max-age=86400` mot nettleseren på `200` og `304` (304 må med — CloudFront sender cache-objektets headere ved revalidering). Edge-cachen styres fortsatt av CARTOs header. Funksjonen deployes automatisk, men virker først når den er manuelt tilknyttet `/tiles/*`-behavioren (se `aws-infrastruktur.md`). Uten funksjonen blir en dårlig tile liggende i nettleseren i et halvt år, og CloudFront-invalidering når ikke nettleserne. Den er en egen funksjon, ikke en response headers policy, fordi en behavior bare kan ha én policy og en kopi av `tot-security-headers` ikke ville blitt oppdatert av CSP-synken.
