...
- [ ] **Deteksjon av stille degradering på kart-tiles** — *ingen plan ennå*
  ...
  - **Merk:** løses «Kart-tiles: selvhost vektor-tiles med Protomaps PMTiles» først, endrer kart-delen av denne karakter — tiles ligger da i vår egen bøtte, så *runtime*-avhengigheten til en tredjepart forsvinner. Men uttrekket stammer fortsatt fra et tredjeparts planetbygg og eldes: et utdatert eller malformert uttrekk kan degradere like stille som et vannmerke. Den generelle delen står uansett
...
- [ ] **Kart-tiles: selvhost vektor-tiles med Protomaps PMTiles** — *ingen plan ennå*
  ...
  - **Fjerner:** tredjeparten og hele GDPR-spørsmålet (ingen IP, ingen `X-Forwarded-For`), API-nøkkelen med placeholder-injeksjon i `setup-cloudfront-functions.mjs` + drift-vakt + `CARTO_API_KEY`-secret, `Referer`-trikset, origin request policy-en `carto-tiles-key-forward`, `/tiles/*`-invalideringssteget og deploy-rekkefølgen. CARTO-attribusjonen faller bort. Attribusjonskravet blir da `© OpenStreetMap contributors` (ODbL) — men Protomaps' basemap-stiler krediteres konvensjonelt ved siden av OSM, så eksakt ordlyd må avklares i planfasen, ikke antas
  - **Koster:** +38 KB gz JS, et nytt vedlikeholdspunkt (oppdatering av uttrekket — realistisk årlig, kan gjøres manuelt), et designvalg fordi Protomaps' stil ikke er CARTO Voyager (må vurderes opp mot design-guiden), og nye tester (`mapInit.test.ts` mocker Leaflet fullstendig i dag)
...
- [ ] **Innholdsdeploy skal ikke blokkeres av nye sikkerhetsavvik** — *ingen plan ennå*
  ...
  - Avveining å ta stilling til i planfasen: gaten er et bevisst supply-chain-vern (fra sikkerhetshardening-oppgaven). Fjernes den på dispatch-stien, kan sårbar kode deployes til prod uten at noen ser det. Motvekten er at koden som deployes uansett er `main`, som allerede har passert gaten ved merge. Motvekt nummer to er svakere enn den ser ut: `Scheduled Security Audit` (`--audit-level=high`, både rot og lambda) kjører kun **ukentlig** (`cron: '0 6 * * 1'`, mandager 06:00 UTC), så et kritisk avvik kan gå i inntil sju dager uten at en *planlagt* audit fanger det. Merk at avviket neppe er *uoppdaget* så lenge — Dependabot-alerten kommer innen timer, og audit-gaten kjører uansett på hver `push` og `pull_request` mot `main`. Forskjellen er at ingen av dem blokkerer dispatch-stien. Vurder derfor om tiltaket bør kombineres med hyppigere scheduled audit
